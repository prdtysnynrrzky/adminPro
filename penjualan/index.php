...
$queryGrafik = "SELECT 
                    produk.nama_produk, 
                    DATE_FORMAT(penjualan.tanggal_penjualan, '%M') AS bulan,
                ...
                GROUP BY 
                    produk.nama_produk, 
                    DATE_FORMAT(penjualan.tanggal_penjualan, '%M')
                ORDER BY 
                    produk.nama_produk, 
                    DATE_FORMAT(penjualan.tanggal_penjualan, '%M')";
...
$labels = [];
$datasets = [];
$colors = ["#cb0c9f", "#3A416F", "#FF5733", "#33FF57", "#3357FF"];

while ($row = $resultGrafik->fetch_object()) {
    $nama_produk = $row->nama_produk;
    $bulan = $row->bulan;
    $total_penjualan = $row->total_penjualan;

    if (!isset($datasets[$nama_produk])) {
        $datasets[$nama_produk] = [
            'label' => $nama_produk,
            'data' => [],
    ...
    $datasets[$nama_produk]['data'][] = $total_penjualan;
                            ...
                            <a href="../profile/index.php" class="nav-link text-body font-weight-bold px-0">
                                ...
                                if (mysqli_num_rows($sqlUser) > 0) {
                                    while ($row = mysqli_fetch_object($sqlUser)) {
                                        ...
                                        <img src="../profile/img/<?= $row->image ?>" alt="avatar" class="rounded-circle"
                                            width="30" height="30">
                                    <?php }
                                }
                                ?>
            ...
            <div class="my-2">
                <div class="card mb-4">
                    <div class="card-header pb-3 d-flex align-items-center justify-content-between">
                        ...
                        <?php
                        $sqlTotalProduk = "SELECT COUNT(*) AS total FROM produk";
                        $resultTotalProduk = $conn->query($sqlTotalProduk);

                        if ($resultTotalProduk->num_rows > 0) {
                            $rowTotalProduk = $resultTotalProduk->fetch_assoc();
                            $total_produk = $rowTotalProduk["total"];
                        } else {
                            $total_produk = 0;
                        }
                        ?>
                        <button type="button" class="btn btn-primary btn-sm bg-gradient-primary ml-auto"
                            data-bs-toggle="modal" data-bs-target="#modalTambah" <?php if ($total_produk == 0)
                                echo "disabled"; ?>>
                            Tambah
                        </button>
                    </div>
                    <div class="card-body px-0 pt-0 pb-2">
                        <div class="table-responsive p-0">
                            <table class="table align-items-center mb-0">
                                ...
                                <tbody>
                                    <?php
                                    $queryPenjualan = "SELECT penjualan.*, produk.image, produk.nama_produk, produk.deskripsi FROM penjualan 
                                        INNER JOIN produk ON penjualan.id_produk = produk.id_produk";
                                    $resultPenjualan = mysqli_query($conn, $queryPenjualan);

                                    if (mysqli_num_rows($resultPenjualan) > 0) {
                                        while ($row = mysqli_fetch_object($resultPenjualan)) {
                                            ?>
                                            <tr>
                                                <td>
                                                ...
                                                <td class="align-middle">
                                                    <span class="text-secondary text-xs font-weight-bold">
                                                        <?php
                                                        $created_at = $row->tanggal_penjualan;
